...
func (s *Service) CheckSmsLimited(ctx context.Context, mobile, clientIP string) (bool, error) {
	// 先限制手机号
	limited, err := s.verify.CheckVerifyFreLimited(ctx, mobile)
	if err != nil || limited {
		return true, err
	}
	// 再限制ip
	limited, err = s.verify.CheckVerifyFreLimited(ctx, clientIP)
	if err != nil || limited {
		return true, err
	}
	return false, nil
